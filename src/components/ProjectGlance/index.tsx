  ...
  return (
    <>
      <style>{`
        ...
        .gold-shimmer {
          background: linear-gradient(90deg, #b89a5e 0%, #e8d5a3 40%, #b89a5e 60%, #8a6e3a 100%);
      ...
      >
        ...
        {/* Top glow line */}
        <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[600px] h-[2px] bg-gradient-to-r from-transparent via-[#b89a5e]/60 to-transparent" />
        ...
        <div className="relative max-w-7xl mx-auto px-5 sm:px-8 lg:px-12">
          ...
          <div className="mb-16 lg:mb-24">
            ...
            >
              <span
                className={`block h-px bg-[#b89a5e] origin-left ${inView ? "animate-lineGrow" : "opacity-0"}`}
                style={{ width: 40, animationDelay: "0.2s" }}
              />
              <span className="text-[#b89a5e] text-[10px] font-semibold tracking-[0.25em] uppercase">
                Project at a Glance
              </span>
            </div>
            ...
            >
              ...
              <span className="text-[#b89a5e] font-medium">20 kathas of prime land</span> — and
              is now open for booking.
            </p>
          </div>

          {/* ── Stats ── */}
          {/* <div className="grid grid-cols-2 sm:grid-cols-4 gap-px bg-[#b89a5e]/15 rounded-sm mb-20 overflow-hidden">
            {stats.map((s, i) => (
              <div
                key={s.label}
              ...
              >
                <span className="text-[#b89a5e]/60">{s.icon}</span>
                <span
                  className="font-display text-[#b89a5e] font-bold leading-none"
                  style={{ fontSize: "clamp(2rem,3.5vw,3rem)" }}
                >
                  {s.value}
                  <span className="text-[55%] ml-1 text-[#b89a5e]/65">{s.unit}</span>
                </span>
                <span className="text-white/35 text-[11px] font-medium tracking-[0.12em] uppercase">
                  {s.label}
                </span>
              </div>
            ))}
          ...
          >
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3 sm:gap-4">
              {floors.map((floor, i) => (
                <div
                  key={floor.id}
                  className={`card-glow group relative bg-[#161410] border border-white/8 hover:border-[#b89a5e]/40 rounded-xl p-5 sm:p-6 flex flex-col gap-3 cursor-default transition-all duration-300 hover:-translate-y-1 ${inView ? "animate-cardIn" : "opacity-0"}`}
                  style={{ animationDelay: `${0.65 + i * 0.08}s` }}
                >
                  {/* Icon */}
                  {/* <span className="text-[#b89a5e]/70 group-hover:text-[#b89a5e] transition-colors duration-300">
                    {floor.icon}
                  </span> */}

                  {/* Floor badge */}
                  <span className="inline-flex self-start items-center px-2.5 py-1 bg-[#b89a5e]/10 border border-[#b89a5e]/25 text-[#b89a5e] text-[10px] font-semibold tracking-[0.14em] uppercase rounded-sm">
                    {floor.floors}
                  </span>
                  ...
                  {/* Hover accent line */}
                  <span className="block w-0 group-hover:w-full h-px bg-gradient-to-r from-[#b89a5e]/60 to-transparent transition-all duration-500 ease-out" />
                </div>
              ))}

              {/* CTA card */}
              <div
                className={`relative bg-[#b89a5e] rounded-xl p-5 sm:p-6 flex flex-col justify-between gap-4 overflow-hidden cursor-pointer transition-all duration-300 hover:-translate-y-1 hover:brightness-110 ${inView ? "animate-cardIn" : "opacity-0"}`}
              ...
              >
                ...
                <a
                  href="/book"
                  className="relative z-10 inline-flex items-center gap-2 bg-[#1a1612] text-[#b89a5e] text-[11px] font-semibold tracking-[0.14em] uppercase px-4 py-2.5 rounded-sm self-start transition-all duration-200 hover:bg-[#2a2018]"
                >
                  Book Now →
                </a>
              </div>
          ...
          {/* ── Footer strip ── */}
          <div
            className={`flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 pt-10 mt-12 border-t border-white/8 ${inView ? "animate-fadeIn" : "opacity-0"}`}
            style={{ animationDelay: "1.3s" }}
          >
            <div className="flex items-center gap-2 text-white/25">
              <HiOutlineLocationMarker size={14} />
              <p className="text-[11px] font-medium tracking-[0.14em] uppercase">
                Tejgaon, Dhaka — Opposite Roads &amp; Highways Department
              </p>
            </div>
            <div className="flex items-center gap-2">
              <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse" />
              <span className="text-emerald-400/80 text-[11px] font-medium tracking-wider uppercase">
                Bookings Open
              </span>
            </div>
          </div>
        ...
        {/* Bottom glow line */}
        <div className="absolute bottom-0 left-1/2 -translate-x-1/2 w-[600px] h-[1px] bg-gradient-to-r from-transparent via-[#b89a5e]/40 to-transparent" />
      </section >
    </>
  );
}